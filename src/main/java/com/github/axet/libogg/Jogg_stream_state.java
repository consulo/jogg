...
public final class Jogg_stream_state {
	...
	public final int ogg_stream_clear() {
		//if( os != null ) {
			clear();
		//}
		return 0;
	...
	public final int ogg_stream_init(final int serialNo) {
		//if( os != null ) {
			clear();
	...
	public final int ogg_stream_check() {
	...
	/*public static int ogg_stream_destroy(Jogg_stream_state os) {
		if( os != null ) {
			ogg_stream_clear( os );
			os = null;
		}
	...
	private final int _os_body_expand(final int needed) {
	...
	private final int _os_lacing_expand(final int needed) {
	...
	private final int ogg_stream_iovecin(final Jogg_iovec_t[] iov, final int count,
					final boolean eos, final long granulePos) {

		int bytes = 0, i;

		if( ogg_stream_check() != 0 ) {
			return -1;
		}
		if( iov == null ) {
			return 0;
		...
		if( _os_body_expand( bytes ) != 0 || _os_lacing_expand( lacing_vals_count ) != 0 ) {
			return -1;
		}
	...
	public final int ogg_stream_packetin(final Jogg_packet op) {
		final Jogg_iovec_t v = new Jogg_iovec_t();
		v.iov_packet = op.packet_base;
		v.iov_base = op.packet;
		v.iov_len = op.bytes;
		final Jogg_iovec_t[] iov = { v };
		return ogg_stream_iovecin( iov, 1, op.e_o_s, op.granulepos );
	}

	/** Conditionally flush a page; force==0 will only flush nominal-size
	pages, force==1 forces us to flush a page regardless of page size
	so long as there's any data available at all. */
	private final int ogg_stream_flush_i(final Jogg_page og, boolean force, final int nfill) {
		if( ogg_stream_check() != 0 ) {
			return 0;
		}
	...
	public final int ogg_stream_flush(final Jogg_page og) {
	...
	public final int ogg_stream_flush_fill(final Jogg_page og, final int nfill) {
		return ogg_stream_flush_i( og, true, nfill );
	}

	/** This constructs pages from buffered packet segments. The pointers
	returned are to static buffers; do not free. The returned buffers are
	good only until the next call (using the same ogg_stream_state) */
	public final int ogg_stream_pageout(final Jogg_page og) {
		boolean force = false;
		if( ogg_stream_check() != 0 ) {
			return 0;
		}
		...
		return ogg_stream_flush_i( og, force, 4096 );
	}

	/** add the incoming page to the stream state; we decompose the page
	  into packet segments here as well. */
	public final int ogg_stream_pagein(final Jogg_page og) {
		if( ogg_stream_check() != 0 ) {
			return -1;
		}
		...
		if( og.ogg_page_serialno() != this.serialno ) {
			return (-1);
		}
		if( og.ogg_page_version() > 0 ) {
			return (-1);
		}
		...
		if( _os_lacing_expand( segments + 1 ) != 0 ) {
			return -1;
		}

		/* are we in sequence? */
		final int page_no = og.ogg_page_pageno();
		...
		boolean bos = og.ogg_page_bos();
		if( og.ogg_page_continued() ) {
...

		{
			...
			if( saved != -1 ) {
				gvals[saved] = og.ogg_page_granulepos();
			}
	...
	page size for applications which are smart enough to provide their
	own delay based flushing */
	public final int ogg_stream_pageout_fill(final Jogg_page og, final int nfill) {
		boolean force = false;
		if( ogg_stream_check() != 0 ) {
			return 0;
		}
		...
		return ogg_stream_flush_i( og, force, nfill );
	}

	public final boolean ogg_stream_eos() {
		if( ogg_stream_check() != 0 ) {
			return true;
		}
		return this.e_o_s;
	...
	public final int ogg_stream_reset() {
		if( ogg_stream_check() != 0 ) {
			return -1;
		}
	...
	public final int ogg_stream_reset_serialno(final int i_serialno) {
		if( ogg_stream_check() != 0 ) {
			return -1;
		}
		ogg_stream_reset();
	...
	private final int _packetout(final Jogg_packet op, final int adv) {
	...
	public final int ogg_stream_packetout(final Jogg_packet op) {
		if( ogg_stream_check() != 0 ) {
			return 0;
		}
	...
	public final int ogg_stream_packetpeek(final Jogg_packet op) {
		if( ogg_stream_check() != 0 ) {
			return 0;
		}
...
/* XXX #ifdef _V_SELFTEST.
		...
		private static Jogg_stream_state os_en = new Jogg_stream_state();
		private static Jogg_stream_state os_de = new Jogg_stream_state();
		private static Jogg_sync_state oy = new Jogg_sync_state();

		private static int sequence = 0;
		private static int lastno = 0;

		@SuppressWarnings("boxing")
		private static void checkpacket(final Jogg_packet op, final long len, final int no, final long pos) {
			int j;
		...
		@SuppressWarnings("boxing")
		private static void check_page(final byte[] data, final int offset, final int[] header, final Jogg_page og) {
			int j;
			// Test data
		...
		@SuppressWarnings("boxing")
		private static void print_header(final Jogg_page og) {
			int j;
			System.err.printf("\nHEADER:\n");
		...
		private static void copy_page(final Jogg_page og) {
		...
		private static void free_page(final Jogg_page og) {
			og.header_base = null;
			og.body_base = null;
			//og.header = -1;
			//og.body = -1;
		...
		private static boolean compare_packet(final Jogg_packet op1, final Jogg_packet op2) {
		...
		private static boolean memcmp(final Jogg_packet p1, final Jogg_packet p2) {
		...
		private static void test_pack(final int[] pl, final int[][] headers, final int byteskip,
			...
			os_en.ogg_stream_reset();
			os_de.ogg_stream_reset();
			oy.ogg_sync_reset();
			...
			for( i = 0; i < packets; i++ ) {
				...
				os_en.ogg_stream_packetin( op );

				// retrieve any finished pages
				{
					final Jogg_page og = new Jogg_page();

					while( os_en.ogg_stream_pageout( og ) != 0 ) {
						...
						{
							final Jogg_page og_de = new Jogg_page();
							final Jogg_packet op_de = new Jogg_packet();
							final Jogg_packet op_de2 = new Jogg_packet();
							final int buf = oy.ogg_sync_buffer( og.header_len + og.body_len );
							int next = buf;
							...
							oy.ogg_sync_wrote( next - buf );

							while( true ) {
								final int ret = oy.ogg_sync_pageout( og_de );
								if( ret == 0 ) {
									break;
								}
								if( ret < 0 )
								...
								os_de.ogg_stream_pagein( og_de );

								// packets out?
								while( os_de.ogg_stream_packetpeek( op_de2 ) > 0 ) {
									os_de.ogg_stream_packetpeek( null );
									os_de.ogg_stream_packetout( op_de );// just catching them all
		...
		public static void main(final String args[]) {
			...
			os_en.ogg_stream_init( 0x04030201 );
			os_de.ogg_stream_init( 0x04030201 );
			oy.ogg_sync_init();
...

			{
				...
				int inptr = 0, i, j;
				final Jogg_page[] og = { new Jogg_page(), new Jogg_page(),
						new Jogg_page(), new Jogg_page(), new Jogg_page() };
				...
				for( i = 0; i < 5; i++ ) {
					if( os_en.ogg_stream_pageout( og[i] ) == 0 ) {
				...
				{
					final Jogg_page temp = new Jogg_page();
					final Jogg_packet test = new Jogg_packet();
					...
					for( i = 0; i < 5; i++ ) {
						System.arraycopy( og[i].header_base, og[i].header,
							oy.data, oy.ogg_sync_buffer( og[i].header_len ), og[i].header_len );
						oy.ogg_sync_wrote( og[i].header_len );
						System.arraycopy( og[i].body_base, og[i].body,
							oy.data, oy.ogg_sync_buffer( og[i].body_len ), og[i].body_len );
						oy.ogg_sync_wrote( og[i].body_len );
					}

					oy.ogg_sync_pageout( temp );
					os_de.ogg_stream_pagein( temp );
					oy.ogg_sync_pageout( temp );
					os_de.ogg_stream_pagein( temp );
					oy.ogg_sync_pageout( temp );
					// skip
					...
					if( os_de.ogg_stream_packetout( test ) != 1 ) {
						error();
					}
				...
				// the rest only test sync
				{
					...
					System.arraycopy( og[1].header_base, og[1].header,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), 3 );
					oy.ogg_sync_wrote( 3 );
					if( oy.ogg_sync_pageout( og_de ) > 0 ) {
						error();
					}

					// Test fractional page inputs: incomplete fixed header
					System.arraycopy( og[1].header_base, og[1].header + 3,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), 20 );
					oy.ogg_sync_wrote( 20 );
					if( oy.ogg_sync_pageout( og_de ) > 0 ) {
						error();
					}
					...
					System.arraycopy( og[1].header_base, og[1].header + 23,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), 5 );
					oy.ogg_sync_wrote( 5 );
					if( oy.ogg_sync_pageout( og_de ) > 0 ) {
						error();
					}
					...
					System.arraycopy( og[1].header_base, og[1].header + 28,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), og[1].header_len - 28 );
					oy.ogg_sync_wrote( og[1].header_len - 28 );
					if( oy.ogg_sync_pageout( og_de) > 0 ) {
						error();
					}

					System.arraycopy( og[1].body_base, og[1].body,
						oy.data, oy.ogg_sync_buffer( og[1].body_len ), 1000 );
					oy.ogg_sync_wrote( 1000 );
					if( oy.ogg_sync_pageout( og_de ) > 0 ) {
						error();
					}

					System.arraycopy( og[1].body_base, og[1].body + 1000,
						oy.data, oy.ogg_sync_buffer( og[1].body_len ), og[1].body_len - 1000 );
					oy.ogg_sync_wrote( og[1].body_len - 1000 );
					if( oy.ogg_sync_pageout( og_de ) <= 0 ) {
						error();
					}
				...
				// Test fractional page inputs: page + incomplete capture
				{
					...
					System.arraycopy( og[1].header_base, og[1].header,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), og[1].header_len );
					oy.ogg_sync_wrote( og[1].header_len );

					System.arraycopy( og[1].body_base, og[1].body,
						oy.data, oy.ogg_sync_buffer( og[1].body_len ), og[1].body_len );
					oy.ogg_sync_wrote( og[1].body_len );
					...
					System.arraycopy( og[1].header_base, og[1].header + 20,
						oy.data, oy.ogg_sync_buffer( og[1].header_len ), og[1].header_len - 20 );
					oy.ogg_sync_wrote( og[1].header_len - 20 );
					System.arraycopy( og[1].body_base, og[1].body,
						oy.data, oy.ogg_sync_buffer( og[1].body_len ), og[1].body_len );
					oy.ogg_sync_wrote( og[1].body_len );
					if( oy.ogg_sync_pageout( og_de ) <= 0 ) {
				...
				// Test recapture: garbage + page
				{
					...
					System.arraycopy( og[2].header_base, og[2].header,
						oy.data, oy.ogg_sync_buffer( og[2].header_len), 20 );
					oy.ogg_sync_wrote( 20 );
					if( oy.ogg_sync_pageout( og_de ) > 0 ) {
						error();
					}
					...
					System.arraycopy( og[2].header_base, og[2].header + 20,
						oy.data, oy.ogg_sync_buffer( og[2].header_len ), og[2].header_len - 20 );
					oy.ogg_sync_wrote( og[2].header_len - 20 );
					System.arraycopy( og[2].body_base, og[2].body,
						oy.data, oy.ogg_sync_buffer( og[2].body_len ), og[2].body_len );
					oy.ogg_sync_wrote( og[2].body_len );
					if( oy.ogg_sync_pageout( og_de ) <= 0 ) {
						error();
					}
				...
				// Test recapture: page + garbage + page
				{
					...
					System.arraycopy( og[2].header_base, og[2].header,
						oy.data, oy.ogg_sync_buffer( og[2].header_len ), og[2].header_len );
					oy.ogg_sync_wrote( og[2].header_len );
					...
					System.arraycopy( og[2].body_base, og[2].body,
						oy.data, oy.ogg_sync_buffer( og[2].body_len ), og[2].body_len - 5 );
					oy.ogg_sync_wrote( og[2].body_len - 5 );

					System.arraycopy( og[3].header_base, og[3].header,
						oy.data, oy.ogg_sync_buffer( og[3].header_len ), og[3].header_len );
					oy.ogg_sync_wrote( og[3].header_len );

					System.arraycopy( og[3].body_base, og[3].body,
						oy.data, oy.ogg_sync_buffer( og[3].body_len ), og[3].body_len );
					oy.ogg_sync_wrote( og[3].body_len );
