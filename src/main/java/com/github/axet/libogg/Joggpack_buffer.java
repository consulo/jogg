...
//#ifdef _V_SELFTEST
	...
	public static void main(final String args[]) {
		...
		o.oggpackB_reset();
		for( i = 0; i < test2size; i++ ) {
			o.oggpackB_write( large[i], 32 );
		}
		buffer = o.oggpackB_get_buffer();
		bytes = o.oggpackB_bytes();
		r.oggpackB_readinit( buffer, bytes );
		for( i = 0; i < test2size; i++ ) {
			if( r.oggpackB_look( 32 ) == -1 ) {
				report("out of data. failed!");
			}
			if( r.oggpackB_look( 32 ) != large[i] ) {
				System.err.printf("%d != %d (%x!=%x):", r.oggpackB_look( 32 ), large[i],
						r.oggpackB_look( 32 ), large[i] );
				report("read incorrect value!\n");
			}
			r.oggpackB_adv( 32 );
		}
		...
		r.oggpackB_readinit( new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 }, 8);
		for( i = 0; i < 64; i++ ) {
			if( r.oggpackB_read( 1 ) != 0 ) {
				System.err.print("failed; got -1 prematurely.\n");
				System.exit( 1 );
			}
		}
		if( r.oggpackB_look( 1 ) != -1 ||
				r.oggpackB_read( 1 ) != -1 ) {
			System.err.print("failed; read past end without -1.\n");
			System.exit( 1 );
		}
		r.oggpackB_readinit( new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 }, 8 );
		if( r.oggpackB_read( 30 ) != 0 || r.oggpackB_read( 16 ) != 0 ){
			System.err.print("failed 2; got -1 prematurely.\n");
			System.exit( 1 );
		}

		if( r.oggpackB_look( 18 ) != 0 ||
				r.oggpackB_look( 18 ) != 0 ) {
			System.err.print("failed 3; got -1 prematurely.\n");
			System.exit( 1 );
		}
		if( r.oggpackB_look( 19 ) != -1 ||
				r.oggpackB_look( 19 ) != -1 ) {
			System.err.print("failed; read past end without -1.\n");
			System.exit( 1 );
		}
		if( r.oggpackB_look( 32 ) != -1 ||
				r.oggpackB_look( 32 ) != -1 ) {
			System.err.print("failed; read past end without -1.\n");
			System.exit( 1 );
		}
		System.err.print("ok.");
		o.oggpackB_writeclear();
