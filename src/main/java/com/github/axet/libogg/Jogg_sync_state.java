 ...
 * <code>int ogg_sync_destroy(Jogg_sync_state oy)</code>
 * -> <code>Jogg_sync_state oy = null</code>
  */
public final class Jogg_sync_state {
	...
	/** @return index to Jogg_sync_state.m_data or -1. */
	public final int ogg_sync_buffer(final int size) {
		if( ogg_sync_check() != 0 )
		{
	...
	public final int ogg_sync_wrote(int bytes) {
		if( ogg_sync_check() != 0 ) {
			return -1;
		}
		bytes += this.fill;
	...
	private final int sync_fail(final int page, int bytes) {
		int i, next;
		this.headerbytes = 0;
		this.bodybytes = 0;
	...
	public final int ogg_sync_pageseek(final Jogg_page og) {
		if( ogg_sync_check() != 0 ) {
			return 0;
		}
		...
		/* The whole test page is buffered. Verify the checksum */
		{
			...
			final Jogg_page log = new Jogg_page();
	...
	public final int ogg_sync_pageout(final Jogg_page og) {
		...
		for( ; ; ) {
			final int ret = ogg_sync_pageseek( og );
			if( ret > 0 ) {
				/* have a page */
				return (1);
			}
